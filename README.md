...
- ⚙️ **自訂設定** — 外環/內圈色彩指標自由切換、色階門檻自訂數值、設定自動存儲
- 🎨 **自訂色階門檻** — 單價/總價分別可設定低/中/高三段門檻值
...
land/
├── web/                          # 🌐 Web 應用
│   ├── server.py                 #    Flask 後端 API (580 行)
│   ├── data_utils.py             #    資料格式化與統計模組 (291 行)
│   └── static/
│       └── index.html            #    前端搜尋+地圖頁面 (1569 行)
...
- **建案群組化**：同建案或同地址的交易自動合併為單一標記
  - 單筆 → 顯示價格圓圈 + 建案名稱
  - 多筆 → 顯示「均XX萬 + N筆」+ 建案名稱（N = 全部交易數量）
  - 點擊多筆標記 → 側欄顯示社區/建案分組列表
- **自訂色階門檻**：設定面板可自訂單價（預設 20/40/70 萬/坪）與總價（預設 500/1500/3000 萬）的色階臨界值
- **雙圈色彩**：
  - 🟢 綠色 ＜ 低門檻
  - 🟡 黃色 ＜ 中門檻
  - 🟠 橙色 ＜ 高門檻
...
- **自動精確定位**：依縮放級別自動切換 OSM 精確座標 / DB 快速座標（預設 ≥ 16 級使用 OSM）
...
### v4.4 (2025-06)
- 🎨 自訂色階門檻：單價/總價各三段門檻值可在設定面板自訂
- 📊 標記顯示全部交易筆數（不再僅顯示近兩年）
- 🏘️ 重疊列表簡化：僅顯示社區/建案分組，移除中位數分析區塊
- 📍 自動精確定位：依縮放級別自動切換 OSM/DB 座標模式（可設定門檻）
- 🗑️ 移除手動切換精確/建案定位按鈕
- 📦 專案架構優化：更新 .gitignore、新增 requirements.txt
- 📝 README 更新

### v4.3 (2025-03)
- 🎯 雙圈 SVG 標記（外環＝總價、內圈＝單價）
- ⚙️ 設定面板（localStorage 自動存儲）
- ⚡ 快速篩選列
- 📱 手機響應式優化
- 🚀 Flask-Compress gzip、lat/lng DB索引、查詢快取
