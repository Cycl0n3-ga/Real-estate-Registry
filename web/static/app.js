...
function initMap(){
  ...
  markerClusterGroup=L.markerClusterGroup({
    ...
    iconCreateFunction:c=>{
      ...
      let sz=40,cls='small';
      if(n>=50){sz=54;cls='large';}else if(n>=10){sz=48;cls='medium';}
      return L.divIcon({
        html:`<div style="width:${sz}px;height:${sz}px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--primary-dark));display:flex;align-items:center;justify-content:center;color:#fff;font-size:${n>999?11:13}px;font-weight:800;box-shadow:0 3px 12px rgba(37,99,235,.4);border:3px solid rgba(255,255,255,.8)">${n}</div>`,
        iconSize:[sz,sz], className:'price-marker'
      });
    }
  });
...
function getHeaderFilterParams(){
  ...
  const hc=document.getElementById('hfCommunity');if(hc&&hc.value.trim())p+='&community='+encodeURIComponent(hc.value.trim());
  return p;
}
...
function hoverTx(idx){
  ...
  map.panTo(targetMarker.getLatLng(),{animate:true,duration:0.25});
  const iconEl=targetMarker._icon;
  if(iconEl){const inner=iconEl.firstElementChild||iconEl;inner.classList.remove('marker-bounce');void inner.offsetWidth;inner.classList.add('marker-bounce');inner.addEventListener('animationend',()=>inner.classList.remove('marker-bounce'),{once:true});}
}
...
function buildGroups(){
  ...
  txData.forEach((tx,idx)=>{
    ...
    if(!markerSettings.showLotAddr&&isLotAddress(tx.address_raw||tx.address||''))return;
    let key;
    if(preciseMode)key='a:'+baseAddress(tx.address_raw||tx.address);
    else if(tx.community_name)key='c:'+tx.community_name;
  ...
  const MERGE=preciseMode?0.00005:0.00025;
  const par=Array.from({length:len},(_,i)=>i);
  function find(x){while(par[x]!==x){par[x]=par[par[x]];x=par[x];}return x;}
  for(let i=0;i<len;i++)for(let j=i+1;j<len;j++)if(Math.abs(arr[i]._cLat-arr[j]._cLat)<MERGE&&Math.abs(arr[i]._cLng-arr[j]._cLng)<MERGE)par[find(i)]=find(j);
  ...
  const merged=Object.values(buckets).map(gs=>{
    ...
    gs.forEach(g=>{m.items.push(...g.items);m.lats.push(...g.lats);m.lngs.push(...g.lngs);m.prices.push(...g.prices);m.unitPrices.push(...g.unitPrices);if(g.label)lbls.push(g.label);});
...
function plotMarkers(fitBounds=true){
  ...
  groups.forEach(g=>{
    ...
    let priceText='';if(avgPriceWan>=10000)priceText=(avgPriceWan/10000).toFixed(1)+'億';else if(avgPriceWan>=1)priceText=Math.round(avgPriceWan)+'萬';else priceText='-';
