...
- v3.3 (2025-02-26)
  - **修正資料庫**：切換到 `land_data_new.db`（8.3M 筆、54% 有座標、38% 有建案名）
  - **雙色環形 Marker**：內圈=總價色（紅/橘/綠）、外環=單價色（深紅/深橘/深綠）
  - **建案座標快取**：啟動時建立 37K+ 建案平均座標快取，無座標記錄自動使用建案平均座標
  - **座標策略改進**：DB 座標 → 建案平均座標 → 行政區座標（移除逐筆 OSM 呼叫，效能提升 100x）
  - **區域搜尋優化**：新增 lat/lng 複合索引，搜尋 500 筆從 ~1s 降至 ~0.06s
  - **com2address 修復**：新 DB 索引 37K+ 建案，精確匹配「健安新城F區」等
  - **Marker 社區標籤**：有建案的 marker 下方顯示建案名（截斷 6 字）
  - **圖例更新**：雙色說明（圓心=總價、外環=單價/坪）
  - **叢集圖標**：叢集也顯示雙色（平均總價填充、平均單價邊框）
  - **MarkerCluster 展開**：高倍率或密集時點擊叢集在側欄顯示交易列表

- v3.2 (2025-02-26)
  - 效能修復：移除逐筆 OSM 地理編碼（500 HTTP 呼叫 → 0）
  - 新增 `_search_by_community_name()` 直查 DB
  - 新增 `_build_filter_where()` 共用篩選子句
  - 新增 `SELECT_COLS` 常數統一欄位

- v3.1 (2025-02-26)
  - UI 改善：卡片設計、叢集偵測、JSON 錯誤處理
  - README 更新
